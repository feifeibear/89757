...
import numpy as np
from prune_utils.pruning import select_top_k, check_sparsity, select_top_k_appr
...
parser.add_argument('--results_dir', metavar='RESULTS_DIR',
                    default='./Results/dgc', help='results dir')
        ...
        if not training:
            ...
            if args.use_residue_acc:
                if args.gpus is not None:
                    ...
                    acc_grad = [torch.zeros(w.size()) for w in list(model.parameters())]
            ...
            for k, mini_input_var in enumerate(mini_inputs):
                ...
                if args.use_residue_acc:
                    ...
                    for u, v, p in zip(U[k], V[k], model.parameters()):
                        ...
                        p_size = np.prod(p.data.size())
                        # print("p_size is ", p_size)
                        if args.use_pruning:
                        #if args.use_pruning:
